...
class NShotTaskSampler(Sampler):
    def __init__(self,
        ...
        if num_tasks < 1:
            raise ValueError('Invalid num_tasks: should be > 1')
        ...
        for _ in range(self.episodes_per_epoch):
            ...
            for task in range(self.num_tasks):
                if self.fixed_tasks is None:
                    # Get some random class
                    episode_classes = np.random.choice(self.dataset.df['class_id'].unique(), size=self.k, replace=False)
                else:
                    # Loop through requested class
                ...
                for k in episode_classes:
                    query = df[(df['class_id'] == l) & (~df['id'].isin(support_k[k]['id']))].sample(self.q)
...
# Arguments
    eval_fn: Callable to perform few-shot classification. Ex: meta_gradient_step
